package com.ecommerce.shopingcart.repository;
...
public class EcommerceRepository {
	...
	public List<Category> getAllCategories() {
		List<Category> items = template.query(" select category_id,  product_category  from category",
				(result, rowNum) -> new Category(result.getInt("category_id"), result.getString("product_category")));
		return items;
	}
	...
	public int insert(OrderCart orderCart) {
		return template.update("insert into ORDERCART (order_id, product_id, date_of_order) " + "values(?,  ?, ?)",
				new Object[] { orderCart.getOrderId(), orderCart.getProductId(), orderCart.getDateOfOrder() });
	}

	public Integer getId() {
		Integer orderId = 0;
		List<OrderCart> items = template.query(
				" select order_id,  product_id, date_of_order from ordercart order by order_id desc limit 1",
				(result, rowNum) -> new OrderCart(result.getInt("order_id"), result.getInt("product_id"),
						result.getString("date_of_order")));
